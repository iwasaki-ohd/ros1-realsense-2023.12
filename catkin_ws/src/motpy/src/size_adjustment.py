...
# "/camera/aligned_depth_to_color/image_rect_raw"のデータを使用]
def CamDepthImageCallback(self, depth_image_data):
        ...
        try:
            ...
            depth_image = self.cv_bridge.imgmsg_to_cv2(depth_image_data, 'passthrough')
...
def realsensecamera_callback(data):
    ...
    # cv2.imshow('Depth', CLmap) # 距離に基づいたカラーマップ
    """"
    if CLmap is not None and CLmap.shape[0] > 0 and CLmap.shape[1] > 0:
        cv2.imshow('Depth', CLmap)
    else:
        rospy.logwarn("Invalid image data. Cannot display.")
        """
...
def main():
    ...
    rospy.Subscriber("/camera/aligned_depth_to_color/image_rect_raw", Image, CamDepthImageCallback)
