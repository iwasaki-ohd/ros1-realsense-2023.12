...
def bounding_boxes_callback(data):
    ...
    if face_box is not None:
        # for文を追加して、複数人に対応できるようにする

        #x = 848
        #y = 479

        # バウンディングボックスの中央座標を計算する
        x_center = int((face_box.xmax + face_box.xmin) / 2 * 435 / 640 + 190) # サイズ調整(*848/640)
        y_center = int((face_box.ymax + face_box.ymin) / 2 * 360 / 360 + 70) # サイズ調整(*480/360)
...
def realsensecamera_callback(data):
    ...
    if face_box is not None:
        x_center = int((face_box.xmax + face_box.xmin) / 2 *480 / 640+ 165)# サイズ調整(*848/640)
        x_rs = int((face_box.xmax + face_box.xmin) / 2)
        y_center = int((face_box.ymax + face_box.ymin) / 2 * 480 / 360 *350/480  + 65) # サイズ調整(*480/360)
    ...
    if depth_image is not None:
        ...
        depth_image = cv2.normalize(depth_image, None, 50, 255, cv2.NORM_MINMAX)
        # 距離の近いものを黒く、遠いものを白くする(値を絶対値に変える)
        depth_image = cv2.convertScaleAbs(depth_image)

    cv2.imshow('G', depth_image)
