...
# 2023.11.28
# motpy_rosとセットで実行するプログラム
# 複数人のIDと距離データを表示可能
# 距離データとｘ座標をパブリッシュする
# カメラを中心とした三次元座標の取得
# 外部パラメータを使用した世界座標の計算
# 世界座標のパブリッシュ(sub_world_point_for_csv用)
# 取得座標のズレを修正  →　無理そうなので一旦放置(11.28)
...
def depth_image_callback(data):
    global face_box, depth_image, CLmap
    CLmap = None
...
def bounding_boxes_callback(data):
    global depth_image, bounding_boxes_list, multiple_data_pub, world_point_pub, CLmap
