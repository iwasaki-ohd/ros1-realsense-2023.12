...
def bounding_boxes_callback(data):
    ...
    if True:
        ...
        #rospy.loginfo("(0, 0): {} [m] \n".format(depth_image[0,0]))
        '''
        a_value = depth_image[Y_test-around:Y_test+around, X_test-around:X_test+around]
        a = np.mean(a_value)/1000
        formatted_a = "{:.3f}".format(a)
        rospy.loginfo("({}, {})周辺3画素の平均距離: {} [m] \n".format(X_test, Y_test, formatted_a))
...
def realsensecamera_callback(data):
    ...
    if True:
        ...
        # cv2.circle(frame, (330, 120), radius=around, color=(0, 255, 0), thickness=around)
        '''
        cv2.circle(frame, (X_test, Y_test), radius=around, color=(0, 255, 0), thickness=around)
